...
def main(env, pnum, signup, confirm, getapikey, delapikey, pwreset, guide,
    ...
    if signup:
        ...
        do_signup(env, pnum, client_name, email)
        return
    if confirm:
        _check_present(client_id, 'client_id')
        _check_present(config, 'config')
        config = read_config(config)
        do_confirm(env, pnum, config['client_id'], config['confirmation_token'])
        return
    if getapikey:
        _check_present(config, 'config')
        config = read_config(config)
        get_api_key(env, pnum, config['client_id'], config['pass'])
    if delapikey:
        _check_present(config, 'config')
        config = read_config(config)
        del_api_key(env, pnum, config['client_id'], config['pass'], delapikey)
        return
    if pwreset:
        _check_present(config, 'config')
        config = read_config(config)
        pw_reset(env, pnum, config['client_id'], config['pass'])
        return
