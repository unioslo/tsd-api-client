...
Upload a single file:
    ...
    tacl2 p11 --upload myfile.txt

Files larger than 1GB are resumable if something goes wrong:

    tacl2 p11 --upload myfile.txt --upload-id 52928fed-8c29-4135-88e9-27f2c0bec526

To browse and manage resumables:

    tacl2 p11 --resume-list
    tacl2 p11 --resume-delete 52928fed-8c29-4135-88e9-27f2c0bec526
    tacl2 p11 --resume-delete-all

Uploading directories (automatically resumable for the whole directory):

    tacl2 p11 --upload mydirectory

To control which folders and files are included:

    tacl2 p11 --upload mydirectory --ignore-prefixes .git,build,dist --ignore-suffixes .pyc,.db

To disable the resume functionality for a directory:

    tacl2 p11 --upload mydirectory --cache-disable

To view and manage directory upload cache:

    tacl2 p11 --upload-cache-show
    tacl2 p11 --upload-cache-delete mydirectory
    tacl2 p11 --upload-cache-delete-all
...
To view files and folders available for download:
    ...
    tacl2 p11 --download-list

Download a file:

    tacl2 p11 --download anonymised-sensitive-data.txt
...
and run the problematic command in verbose mode:
    ...
    tacl2 --version
    tacl2 p11 --verbose --upload myfile # e.g.
